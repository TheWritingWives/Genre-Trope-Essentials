...
def grant_lifetime_access(email=None):
    """Unlocks every tool in one shot."""
    st.session_state["lifetime_access"]       = True
    st.session_state["access_granted"]        = True
    st.session_state["access_reason"]         = "lifetime"
    st.session_state["ad_access_granted"]     = True
    st.session_state["ad_access_reason"]      = "lifetime"
    st.session_state["cover_access_granted"]  = True
    st.session_state["amazon_access_granted"] = True
    if email:
        st.session_state["lifetime_email"] = email
...
<style>
  ...
  .stLinkButton > a, [data-testid="stPageLink"] a {{
    background:{PRIMARY} !important; color:{GOLD} !important;
    font-weight:700 !important; border-radius:8px !important;
    font-size:0.9rem !important; border:2px solid {GOLD} !important;
    text-decoration:none !important; display:block !important;
    padding:0.55rem 1.2rem !important; text-align:center !important;
  }}
  /* Community CTA */
  .community-card {{
    background: {PRIMARY}; border: 2px solid {GOLD};
